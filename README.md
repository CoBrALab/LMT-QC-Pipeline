...
- **Extracted, not reimplemented.** Every function and constant in this module is a verbatim move from `2.lmt_binary_search.py`'s original copy (the first of the three to exist); nothing was rewritten or altered during extraction, to keep this refactor behavior-invariant.
...
- **Any change to `get_start_frame()`, `build_video_map()`, or `find_nearest_frame_candidates()` changes frame resolution for all three calling scripts simultaneously** — that's the entire point of this module, but it also means a change here has no "blast radius" containment the way editing one script's local copy used to.
