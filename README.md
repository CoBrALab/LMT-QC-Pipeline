...
- `IN_NEST = −1` sentinel must remain −1; downstream scripts (3B, 5B) filter on this exact value
- Table name `GAP_FILL_ANALYSIS` — hardcoded in 3B's read query
...
- Outputs a complete frame classification table preserving all rows from the 1B input, with a `BINARY_SEARCH` column added.
...
**Outputs:**
- `lmt_binary_search_<date>.sqlite` — table `GAP_FILL_ANALYSIS`; all rows from 1B input preserved, `IN_NEST` updated for resolved frames, `BINARY_SEARCH` column (0/1) added
...
- `BINARY_SEARCH = 1` is set only for frames in searchable gaps (above threshold); this distinction is used by 5B
...
### Script: `lmt_qc_sampler.py`

**Purpose:**
- Randomly samples frames from the `lmt_binary_search.py` output and extracts corresponding video screenshots for manual QC.
...
- LMT video files (.mp4) — same naming convention as required by 3B
- Output folder path
- Animal ID (integer)
...
**Do NOT modify:**
- `FRAME_CONVERSION = 2` — must match the value in 3B
- Video filename frame-number parsing — must match 3B convention
- `QC_TYPE` column written to output — 5B reads this to select the correct filter; changing the string values breaks the handoff
- Table name `QC_ASSUMED_SAMPLES` — hardcoded in 5B's read query
...
- `BINARY_SEARCH` column defaults to 0 if absent — provides backward compatibility with pre-update 3B outputs, but such outputs also lack detected rows, so the detected QC pool will be empty

---

### Script: `lmt_qc_validator.py`

**Purpose:**
- Presents sampled frames from `lmt_qc_sampler.py` output for manual labelling and computes a confusion matrix comparing algorithm classifications against human labels.

**Inputs:**
- `lmt_qc_sampler_<slug>_<timestamp>.sqlite` (table: `QC_ASSUMED_SAMPLES`)
- Screenshot folder produced by the corresponding 4B run
...
**Do NOT modify:**
- `QC_TYPE` string constants — must match the values written by 4B (`"Assumed Rows QC"`, `"LMT Detected QC"`)
...
**Open-source notes:**
- The screenshot path is reconstructed as `os.path.join(screenshot_folder, row["screenshot"])`; the screenshot folder must be the exact folder produced by the corresponding 4B run — mismatching SQLite and folder (e.g. from different runs) will cause missing-file errors
...
- Legacy 4B outputs (no `QC_TYPE` column) are supported with a fallback warning but will only work correctly if the file contains assumed rows; feeding a legacy detected-only file produces an empty session
